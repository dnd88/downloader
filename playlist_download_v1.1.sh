...
# sed -i -e 's/\r$//' [대상 파일]
# 플레이리스트 여러개 다운로드 경우, 주석 풀고 사용하기 :<<'END' 와 END
...
SET=$(seq 23 56)
for i in $SET; do
  youtube-dl -f 'bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best' -o './download_video/%(title)s.%(ext)s' "https://www.youtube.com/watch?v=j53iE8w6EuY&list=PL8uzAiCio-SHQoM1yJ-jwzHHN3-4AAGWg&index="$i
  echo "https://www.youtube.com/watch?v=j53iE8w6EuY&list=PL8uzAiCio-SHQoM1yJ-jwzHHN3-4AAGWg&index="$i
done
echo "Finish"
